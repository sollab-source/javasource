...
class Outer2 {
	...
	static class InnerStaticClass{    // 내부 클래스 선언
		int iNum = 100;               // 내부 클래스 인스턴스 변수
		static int sNum = 200;        // 내부 클래스 정적 변수
		
		void method() {
			// iPos += 10;            // 에러 발생(내부정적 클래스에서 외부 인스턴스 변수 사용 불가) 			
...
public class InnerEx2 {
	public static void main(String[] args) {		
		...
		inner.method();                               // 내부 클래스 참조 변수로 인스턴스 메소드 호출
		Outer2.InnerStaticClass.method2();	     // 정적 내부 클래스 참조 변수로 인스턴스 메소드 호출
	}
}
